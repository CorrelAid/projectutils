...
test_that("test that non-existing folder throws error", {
  expect_error(load_project("foo", data_folder = "tests/testthat/test_data"), regexp = "foo does not exist.", class = "usethis_error")
})

test_that("test that loading project works", {
  p <- load_project("2020-01-TES", data_folder = "tests/testthat/test_data")
  expect_equal(is.list(p), TRUE)
})

# update project
test_that("test that non-existing field throws error", {
    project <- load_project("2020-01-TES", data_folder = "tests/testthat/test_data")
