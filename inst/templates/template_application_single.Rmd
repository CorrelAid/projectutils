...
tmp_all <- appl %>% 
  ...
  arrange(applicant_id, question, category)
...
**Applied to project(s)**: `r a$applied_to`
...
ggplot(tmp_applicant, aes(x = fct_rev(category), y = level_num, group = question)) +
  geom_segment(aes(
    x = fct_rev(category),
...
tmp_all_highlight <- tmp_all %>%
  ...
  arrange(question, category) %>% # this is critical so that the bar plots are correctly ordered (through fct_inorder)
  mutate(
...
ggplot(tmp_all_highlight, aes(
  ...
  facet_wrap(~fct_inorder(category), ncol = 1, strip.position = "left") +
...
ggplot(tmp_all_highlight, aes(
  ...
  facet_wrap(~fct_inorder(category), ncol = 4) +
  theme_grey() +
  theme(
    panel.grid = element_blank(),
    panel.grid.major.y = element_line("grey92"),
