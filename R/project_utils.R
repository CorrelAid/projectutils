...
load_project <- function(project_id_path, data_folder = here::here()) {
  ...
  j <- jsonlite::read_json(here::here(data_folder, project_id_path, "meta.json"))
  ...
  set_description_data <- function(lang) {
    ...
    j$organization$about[[lang]][["text"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "00_about.md"))

    
    # hardcoded to avoid loading any "junk" and to get better error messages
    j$description[[lang]][["summary"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "00_summary.md"))
    j$description[[lang]][["problem"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "01_problem.md"))
    j$description[[lang]][["data"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "02_data.md"))
    j$description[[lang]][["approach"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "03_approach.md"))
    j$description[[lang]][["impact"]] <- readr::read_file(here::here(data_folder, project_id_path, lang, "04_impact.md"))
    return(j)
  }
