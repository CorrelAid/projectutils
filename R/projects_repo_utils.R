    ...
    cards <- columns %>%
        purrr::map_dfr(function(column) {
            ...
            tibble::tibble(name = column$name, issue_url = issue_urls) %>%
                dplyr::mutate(issue_num = stringr::str_replace_all(issue_url, ".+?/(\\d{1,})$", "\\1"))
        })
...
get_gh_issue_data <- function(project_id) {
    ...
    labels_df <- gh_issue$labels %>%
        map_chr("name") %>%
        tibble::tibble(name = .) %>%
        tidyr::separate(name, into = c("category", "value"), sep = ":", fill = "right")
    ...
    return(
        list(
            name = name, 
            tags = labels_df %>% filter(category != 'lc'), 
            lc = lcs,
